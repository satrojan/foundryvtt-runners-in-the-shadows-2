...
export class BladesActorSheet extends BladesSheet {
  ...
  /** @override */
    ...
    html.find('.debug-toggle').click(async ev => {
      let debug = await this.actor.getFlag('runners-in-the-shadows', 'show-debug');
      await this.actor.setFlag('runners-in-the-shadows', 'show-debug', !debug);
    });
    ...
    html.find('.toggle-allow-edit').click(async (event) => {
      ...
      if(this.actor.getFlag('runners-in-the-shadows', 'allow-edit')){
        await this.actor.unsetFlag('runners-in-the-shadows', 'allow-edit');
      } else {
        await this.actor.setFlag('runners-in-the-shadows', 'allow-edit', true);
      }
    });
    ...
    html.find('.harm-box').click(ev => {
      this.actor.getFlag('runners-in-the-shadows', 'harm_open') ? this.actor.setFlag('runners-in-the-shadows', 'harm_open', false) : this.actor.setFlag('runners-in-the-shadows', 'harm_open', true);
    });
    ...
    html.find('.load-box').click(ev => {
      this.actor.getFlag('runners-in-the-shadows', 'load_open') ? this.actor.setFlag('runners-in-the-shadows', 'load_open', false) : this.actor.setFlag('runners-in-the-shadows', 'load_open', true);
    });
